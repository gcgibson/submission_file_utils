...
for (location in unique_locations){
  plotdf <- ggplot(submission_file_point[submission_file_point$location == location,],aes(x=target_end_date,y=value)) + geom_point() + 
    ...
    geom_text(data=y_states_date_max_diffs[y_states_date_max_diffs$location == location,],aes(x=tail(y_states_date_subset$date,1)[1],y=mean(submission_file_point[submission_file_point$location == location,]$value)/2,label=paste0(diff)))+
